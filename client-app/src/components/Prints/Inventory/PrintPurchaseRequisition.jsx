...
const DocumentBody = ({ header, lines }) => {
  const { requestedBy, requestDate, approvalStatus, approvedBy, approvedDate } =
    header;

  const RenderLines = (data) => {
    const { inventoryMaster, lineQuantity, note } = data;
    return (
  ...
  return (
    <Document title="Guest Folio">
      <Page size="A4" orientation="landscape" style={styles.body}>
        ...
        <View>
          <Text style={styles.titleText}>Supply Requisition</Text>
        </View>
...
const PrintPurchaseRequisition = () => {
  ...
  useEffect(() => {
    ...
    async function getData() {
      try {
        const { data } = await GetPurchaseReqHeaderAndLine(headerId);
        const { headerDto, lines } = data.singleRecord;
        if (isMounted()) {
          setHeaderData(headerDto);
          setLineData(lines);
        }
      } catch (error) {
        console.log(error);
...
export default PrintPurchaseRequisition;
